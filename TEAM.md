...
- Quyết định, khó khăn và cách xử lý: Bổ sung sau, do Minh tự viết theo [RULES.md](RULES.md) Quyết định tập trung UI vào việc hiển thị rõ toàn bộ luồng xử lý gồm input, tool call, kết quả/lỗi, trạng thái lượt và artifact_version để thuận tiện theo dõi và debug. Khó khăn chính là đảm bảo UI hoạt động ổn định khi tích hợp với backend qua HTTP và xử lý các trường hợp lỗi. Em xử lý bằng cách bổ sung HTTP smoke test, xây dựng các kịch bản hội thoại cố định và lưu transcript để đối chiếu kết quả thực tế với yêu cầu trong RULES.md.
- Điều đã học: [RULES.md](RULES.md) Hiểu rõ hơn cách tích hợp UI với backend/tool calling và cách thiết kế giao diện có khả năng quan sát được quá trình xử lý của hệ thống AI. Đồng thời học được cách sử dụng smoke test và transcript để kiểm thử, debug và lưu bằng chứng thay vì chỉ kiểm tra thủ công. Em cũng hiểu thêm tầm quan trọng của việc thống nhất input, output, trạng thái và version khi tích hợp nhiều thành phần trong một sản phẩm..
- AI/công cụ đã dùng và cách kiểm tra: Bổ sung sau, do Minh tự viết theo [RULES.md](RULES.md) Sử dụng AI để hỗ trợ đọc hiểu code, đề xuất cách triển khai UI và xử lý lỗi trong quá trình tích hợp. Sử dụng Python, HTTP smoke test và các transcript scenario để kiểm tra. Các đề xuất từ AI đều được kiểm tra lại bằng cách chạy trực tiếp trên project, đối chiếu output thực tế, kiểm tra trên web UI và đối chiếu với yêu cầu trong RULES.md.
- Thời điểm đã tự nộp URL repo chung trên VLearn: 21h 15/9/2026
